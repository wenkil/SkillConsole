...
const copy = {
  en: {
    ...
    transitionBreakdown: "Result transition breakdown",
...
function htmlSubject(
  ...
  const labels = locale === "zh-CN"
    ? { kind: "类型", version: "版本", manifest: "清单", scripts: "脚本" }
    : { kind: "Kind", version: "Version", manifest: "Manifest", scripts: "Scripts" }
  return `<article class="subject"><span>${escapeHtml(label)}</span><h3>${escapeHtml(subject.label)}</h3><dl><div><dt>${labels.kind}</dt><dd>${escapeHtml(subject.kind)}</dd></div><div><dt>${labels.version}</dt><dd>${escapeHtml(subject.versionNumber === null ? "—" : `R${subject.versionNumber}`)}</dd></div><div><dt>${labels.manifest}</dt><dd><code>${escapeHtml(subject.manifestHash?.slice(0, 16) ?? "—")}</code></dd></div><div><dt>${labels.scripts}</dt><dd>${subject.declaredBundledScripts.length}</dd></div></dl></article>`
}
...
export const testReportDocumentStyles = `:root{color-scheme:light;--ink:#191816;--muted:#6e675f;--paper:#f5f1e8;--raised:#fffdf7;--rule:#c8c0b4;--accent:#e86041;--good:#28734f;--bad:#b33b32;--warn:#966414}*{box-sizing:border-box}html{background:var(--paper);color:var(--ink);font-family:Inter,"Segoe UI",sans-serif;font-size:16px;line-height:1.65}body{margin:0}a{color:inherit;text-decoration-thickness:1px;text-underline-offset:3px}main{width:min(1120px,calc(100% - 48px));margin:0 auto;padding:40px 0 80px}.hero{border:1px solid var(--ink);background:var(--raised);padding:32px}.eyebrow,.meta,dt,.pill,th{font-family:"SFMono-Regular",Consolas,monospace}.eyebrow{color:var(--accent);font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.hero h1{font-size:clamp(32px,5vw,56px);line-height:1.04;margin:12px 0 18px;letter-spacing:-.045em}.hero-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:22px}.hero-actions a{border:1px solid var(--ink);padding:9px 13px;background:var(--paper);font-size:14px;font-weight:700}.meta{display:flex;flex-wrap:wrap;gap:8px 18px;color:var(--muted);font-size:12px}.notice{border-left:4px solid var(--accent);background:var(--paper);padding:14px 16px}.warning{border-color:var(--warn)}section{margin-top:34px}section>h2{border-bottom:2px solid var(--ink);padding-bottom:9px;font-size:24px;line-height:1.25}.subjects,.metric-grid,.case-grid,.environment-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.subject,.metric-card{border:1px solid var(--rule);background:var(--raised);padding:18px}.subject>span,.metric-card>span{font-family:Consolas,monospace;color:var(--muted);font-size:11px;text-transform:uppercase}.subject h3{font-size:20px;line-height:1.3;margin:7px 0 14px}.subject dl,.case-grid dl,.environment-grid dl{margin:0}.subject dl div,.case-grid dl div,.environment-grid dl div{display:grid;grid-template-columns:132px 1fr;gap:12px;border-top:1px solid var(--rule);padding:8px 0}.subject dt,.case-grid dt,.environment-grid dt{font-size:11px;color:var(--muted)}dd{margin:0;overflow-wrap:anywhere}.metric-card strong{display:block;font-size:30px;line-height:1.1;margin:8px 0 4px}.metric-card small{color:var(--muted);font-size:13px}table{width:100%;border-collapse:collapse;background:var(--raised);font-size:14px}th,td{border:1px solid var(--rule);padding:10px;text-align:left;vertical-align:top}th{font-size:11px}.metrics-table td strong,.metrics-table td small{display:block}.metrics-table td small{color:var(--muted);margin-top:3px}.pill{display:inline-block;border:1px solid var(--rule);padding:3px 7px;font-size:10px;font-weight:700}.status-PASSED{color:var(--good);border-color:var(--good)}.status-FAILED,.status-EXECUTION_ERROR,.status-ASSESSMENT_ERROR{color:var(--bad);border-color:var(--bad)}.status-INSUFFICIENT_EVIDENCE{color:var(--warn);border-color:var(--warn)}.case{border:1px solid var(--rule);background:var(--raised);margin-bottom:12px}.case summary{display:grid;grid-template-columns:78px minmax(180px,1fr) auto auto;align-items:center;gap:12px;cursor:pointer;padding:16px;font-size:14px}.case summary::marker{color:var(--accent)}.case-body{border-top:1px solid var(--rule);padding:18px}.case-body table{margin-top:16px}.evidence-link{text-align:right}.technical-details{border:1px solid var(--rule);background:var(--raised)}.technical-details>summary{cursor:pointer;padding:16px 18px;font-weight:750}.technical-details>summary::marker{color:var(--accent)}.technical-details>div{border-top:1px solid var(--rule);padding:18px}.technical-details h3{font-size:17px;margin:0 0 12px}.environment-grid>div{border:1px solid var(--rule);background:var(--paper);padding:16px}.environment-grid h3{margin-top:0}code{font-family:Consolas,monospace;font-size:.9em;overflow-wrap:anywhere}.footer{margin-top:42px;border-top:1px solid var(--ink);padding-top:16px;color:var(--muted);font-size:12px}@media(max-width:760px){html{font-size:15px}main{width:min(100% - 24px,1120px);padding-top:16px}.hero{padding:20px}.subjects,.metric-grid,.case-grid,.environment-grid{grid-template-columns:1fr}.case summary{grid-template-columns:64px 1fr}.case summary>*:nth-child(n+3){display:none}.table-scroll{overflow-x:auto}table{min-width:650px}}@media print{main{width:100%;padding:0}.hero-actions{display:none}.case{break-inside:avoid}details:not([open])>.case-body{display:block}.technical-details:not([open])>div{display:block}.hero,section{break-inside:avoid}}`

export function renderTestReportHtmlFragment(
  report: StructuredTestReportV1,
  locale: TestReportDocumentLocale,
): string {
  const t = copy[locale]
...
<section><details class="technical-details"><summary>${escapeHtml(t.technicalDetails)}</summary><div><h3>${escapeHtml(t.transitionBreakdown)}</h3><div class="table-scroll"><table><tbody>${transitionRows || `<tr><td>${escapeHtml(t.noData)}</td></tr>`}</tbody></table></div><h3 style="margin-top:28px">${escapeHtml(t.environment)}</h3>${htmlEnvironment(report, locale)}<h3 style="margin-top:28px">${escapeHtml(t.traceability)}</h3><div class="table-scroll"><table><tbody>${traceability}</tbody></table></div></div></details></section>
